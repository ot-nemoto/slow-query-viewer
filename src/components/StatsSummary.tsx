...
import { SlowQueryEntry } from '@/lib/slowQueryParser';
...
export default function StatsSummary({ entries }: StatsSummaryProps) {
  ...
  const totalRowsExamined = entries.reduce((sum, entry) => sum + entry.rowsExamined, 0);
  ...
  const timeRange = entries.length > 1 ? {
    start: new Date(Math.min(...entries.map(e => new Date(e.time).getTime()))),
    end: new Date(Math.max(...entries.map(e => new Date(e.time).getTime())))
  } : null;
    ...
    <div className="bg-white rounded-lg shadow-md p-6 mb-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-yellow-50 rounded-lg p-4">
          <div className="text-2xl font-bold text-yellow-600">{formatTime(avgTime)}</div>
          <div className="text-sm text-gray-600">平均実行時間</div>
        </div>

        <div className="bg-red-50 rounded-lg p-4">
          <div className="text-2xl font-bold text-red-600">{formatTime(maxTime)}</div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {timeRange && (
          <div>
            ...
            <div className="text-sm text-gray-600">
              <div><strong>開始:</strong> {formatDate(timeRange.start)}</div>
              <div><strong>終了:</strong> {formatDate(timeRange.end)}</div>
              <div><strong>期間:</strong> {Math.ceil((timeRange.end.getTime() - timeRange.start.getTime()) / (1000 * 60 * 60 * 24))} 日</div>
            </div>
          </div>
        )}
        ...
        <div>
          ...
          <div className="text-sm text-gray-600">
            <div><strong>平均検索行数:</strong> {avgRowsExamined.toFixed(0).toLocaleString()}</div>
            <div><strong>最小実行時間:</strong> {formatTime(minTime)}</div>
            <div><strong>実行時間範囲:</strong> {formatTime(minTime)} - {formatTime(maxTime)}</div>
          </div>
        </div>
      </div>
      ...
      {slowestQuery && (
        <div className="mt-6 p-4 bg-red-50 rounded-lg">
          ...
          <div className="text-sm text-gray-700 grid grid-cols-2 gap-4 mb-2">
            <div><strong>実行時間:</strong> {formatTime(slowestQuery.queryTime)}</div>
            <div><strong>実行時刻:</strong> {formatDate(new Date(slowestQuery.time))}</div>
            <div><strong>ユーザー:</strong> {slowestQuery.user}</div>
            <div><strong>検索行数:</strong> {slowestQuery.rowsExamined.toLocaleString()}</div>
          </div>
          <div className="mt-2">
            <strong>クエリ:</strong>
